...
export default function SectorDrop(){
  const audio=useGameAudio(); const [board,setBoard]=useState<Cell[][]>(empty); const [piece,setPiece]=useState<Piece>(()=>firstPiece(0)); const [next,setNext]=useState<Piece>(()=>firstPiece(1)); const [score,setScore]=useState(0); const scoreRef=useRef(0); const [lines,setLines]=useState(0); const [best,setBest]=useState(0); const [phase,setPhase]=useState<"ready"|"playing"|"over">("ready");
  ...
  const restart=useCallback(()=>{audio.play("navigate");scoreRef.current=0;setBoard(empty());setPiece(randomPiece());setNext(randomPiece());setScore(0);setLines(0);setPhase("playing")},[audio]);
  const lock=useCallback((current:Piece)=>{let merged=board.map(row=>[...row]);current.shape.forEach((row,y)=>row.forEach((v,x)=>{const yy=current.y+y;if(v&&yy>=0)merged[yy][current.x+x]=current.color}));const kept=merged.filter(row=>row.some(v=>!v));const cleared=H-kept.length;merged=[...Array.from({length:cleared},()=>Array<Cell>(W).fill(null)),...kept];if(cleared){audio.play(cleared>=3?"success":"complete");setLines(v=>v+cleared);scoreRef.current+=[0,100,280,520,900][cleared];setScore(scoreRef.current)}else audio.play("tap");setBoard(merged);const incoming={...next,x:Math.floor((W-next.shape[0].length)/2),y:-1};setNext(randomPiece());if(hit(merged,incoming,0,1)){audio.play("error");setPhase("over");setBest(old=>{const value=Math.max(old,scoreRef.current);try{localStorage.setItem("fairbyte:sector-drop:best",String(value))}catch{}return value})}else setPiece(incoming)},[audio,board,next]);
  ...
  const spin=useCallback(()=>{if(phase!=="playing")return;const turned=rotate(piece.shape);for(const kick of [0,-1,1,-2,2])if(!hit(board,piece,kick,0,turned)){setPiece({...piece,x:piece.x+kick,shape:turned});audio.play("tap");return}},[audio,board,phase,piece]);
  const hardDrop=useCallback(()=>{if(phase!=="playing")return;let y=piece.y;while(!hit(board,{...piece,y},0,1))y++;scoreRef.current+=Math.max(0,y-piece.y)*2;setScore(scoreRef.current);lock({...piece,y})},[board,lock,phase,piece]);
