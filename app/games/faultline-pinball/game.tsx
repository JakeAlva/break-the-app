...
const W=720,H=760;
const SHOOTER_INNER_X=606,SHOOTER_GATE_Y=162,SHOOTER_EXIT_Y=96;
...
export default function FaultlinePinball(){
  ...
  const releasePlunger=useCallback(()=>{input.current.launch=false;const w=world.current,held=w.balls.find(ball=>ball.held);if(w.phase!=="playing"||!held)return;held.held=false;held.vx=-.08;held.vy=-(17.8+w.charge*.055);w.charge=0;w.lastChargeUi=-1;sync();audio.play("success")},[audio,sync]);
  ...
  useEffect(()=>{let frame=0,last=performance.now();const ctx=canvas.current?.getContext("2d");if(!ctx)return;
    const award=(value:number)=>{const w=world.current;w.combo++;w.comboClock=105;const comboFactor=Math.min(4,1+Math.floor(w.combo/8));w.score+=value*w.multiplier*comboFactor;w.quake=Math.min(100,w.quake+value/220);w.flash=5;if(w.quake>=100&&w.balls.filter(ball=>!ball.held).length===1){w.quake=0;w.balls.push({...makeBall(false),x:330,y:320,vx:-5.3,vy:-7.5},{...makeBall(false),x:385,y:330,vx:5.1,vy:-7.1});w.multiplier=Math.min(8,w.multiplier+1);w.shake=42;audio.play("complete")}sync()};
    ...
    const loop=(now:number)=>{const dt=Math.min(1.7,(now-last)/16.67);last=now;const w=world.current;
      if(w.phase==="playing"){
        ...
        const substeps=4,step=dt/substeps;
        for(let sub=0;sub<substeps;sub++)for(const ball of w.balls){if(!ball.live||ball.held)continue;ball.hitLock=Math.max(0,ball.hitLock-step);ball.vy+=.235*step;ball.vx*=Math.pow(.9992,step);ball.vy*=Math.pow(.9992,step);ball.x+=ball.vx*step;ball.y+=ball.vy*step;
          ...
          if(ball.x>600&&ball.y<SHOOTER_EXIT_Y&&ball.vy<0){if(!ball.skill){ball.skill=true;award(750);audio.play("good")}ball.x=592;ball.vx=-7.2-Math.min(2.4,Math.abs(ball.vy)*.08);ball.vy=1.8}
          if(ball.skill&&ball.x>598&&ball.y<SHOOTER_GATE_Y&&ball.vx>0){ball.x=598;ball.vx=-Math.abs(ball.vx)*.84;ball.vy+=.6}
          if(ball.y>SHOOTER_GATE_Y&&ball.y<712)segmentCollision(ball,{x:SHOOTER_INNER_X,y:SHOOTER_GATE_Y},{x:SHOOTER_INNER_X,y:712},5,.9);
          segmentCollision(ball,{x:44,y:480},{x:188,y:577},6,.82);segmentCollision(ball,{x:SHOOTER_INNER_X,y:521},{x:512,y:577},6,.82);
          ...
          const speed=Math.hypot(ball.vx,ball.vy);if(speed>18){ball.vx=ball.vx/speed*18;ball.vy=ball.vy/speed*18}
      ...
      ctx.fillStyle="#09060d";ctx.fillRect(612,SHOOTER_GATE_Y,59,546);ctx.strokeStyle="#09060d";ctx.lineWidth=52;ctx.beginPath();ctx.arc(SHOOTER_INNER_X,SHOOTER_GATE_Y,70,-Math.PI/2,0);ctx.stroke();ctx.strokeStyle="#7ffcff";ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(SHOOTER_INNER_X,SHOOTER_GATE_Y);ctx.lineTo(SHOOTER_INNER_X,712);ctx.stroke();ctx.beginPath();ctx.arc(SHOOTER_INNER_X,SHOOTER_GATE_Y,70,-Math.PI/2,0);ctx.stroke();ctx.strokeStyle="#fd75cf55";ctx.strokeRect(618,164,47,540);ctx.save();ctx.translate(647,468);ctx.rotate(-Math.PI/2);ctx.fillStyle="#7ffcff99";ctx.font="900 11px Arial";ctx.letterSpacing="2px";ctx.fillText("LAUNCH  ↑",0,0);ctx.restore();
      ctx.strokeStyle="#7ffcff";ctx.lineWidth=4;ctx.lineCap="round";ctx.beginPath();ctx.moveTo(44,480);ctx.lineTo(188,577);ctx.stroke();ctx.beginPath();ctx.moveTo(SHOOTER_INNER_X,521);ctx.lineTo(512,577);ctx.stroke();
