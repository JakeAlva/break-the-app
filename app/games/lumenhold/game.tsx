...
export default function Lumenhold(){
  ...
  useEffect(()=>{const timer=window.setTimeout(()=>{try{setBest(Number(localStorage.getItem("fairbyte:lumenhold:best")||0))}catch{}},0);return()=>window.clearTimeout(timer)},[]);
  ...
  useEffect(()=>{let request=0,last=performance.now();const ctx=canvas.current?.getContext("2d");if(!ctx)return;const tick=(now:number)=>{const dt=Math.min(2,(now-last)/16.67);last=now;const w=world.current;if(w.phase==="wave"){
      ...
      const defeated=w.enemies.filter(enemy=>enemy.hp<=0);if(defeated.length){const splits:Enemy[]=[];for(const enemy of defeated){w.light+=enemy.reward;w.score+=enemy.reward*10;if(enemy.type==="splitter")for(const offset of [-10,10])splits.push({id:w.nextId++,type:"runner",progress:Math.max(0,enemy.progress+offset),hp:Math.max(8,Math.round(enemy.max*.34)),max:Math.max(8,Math.round(enemy.max*.34)),speed:1.08,slow:enemy.slow,burn:0,reward:6})}w.enemies=[...w.enemies.filter(enemy=>enemy.hp>0),...splits];sync()}w.shots.forEach(shot=>shot.life-=dt);w.shots=w.shots.filter(shot=>shot.life>0);
